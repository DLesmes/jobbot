...
#storage
from app.utils import save_json, open_json
...
def linkedin_scraper():
    scraper = LinkedinScraper(chrome_executable_path='/usr/local/bin/chromedriver', #'/usr/bin/chromedriver', # Custom Chrome executable path (e.g. /foo/bar/bin/chromedriver) 
...
def pilot():
    ...
    def on_data(data: EventData):
        ...
        jobs_data = {'vacancy_name':data.title, #title
    ...
    scraper.on(Events.END, save_json(settings.RESULTS, []))

    
    info = f"[PILOT] running scraping loop]"
    logging.info(info)
    map_countries_keywords = dict(open_json(settings.MAP_COUNTRIES_KEYWORDS))
    for country, search in map_countries_keywords.items():
        for cargo, num_vacantes in search.items():
            print(
                'CARGO.################',
                cargo
            )
